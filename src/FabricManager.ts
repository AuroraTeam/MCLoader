...
export class FabricManager extends MojangManager {
    ...
    async downloadClient(clientVer: string, dirName: string): Promise<void> {
        ...
        await super.downloadClient(clientVer, dirName, true)

        const librariesDir = path.resolve(StorageHelper.filesDir, dirName, "libraries")

        LogHelper.info("Библиотеки Fabric загружаются, пожалуйста подождите...")
